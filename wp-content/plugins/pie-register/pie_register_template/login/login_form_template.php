...
if(!class_exists("Login_form_template"))
{
	class Login_form_template
	{
		...
		function add_username(){
			...
			$form_data .= '<input placeholder="'.((isset($this->pr_option['login_username_placeholder']) && !empty($this->pr_option['login_username_placeholder']))? __($this->pr_option['login_username_placeholder'],"piereg") : "").'" type="text" value="'.$user_name_val.'" class="input input_fields piereg_validate[required]" id="user_login" name="log">';
			$form_data .= '</div>';
			return $form_data;
		}
		function add_password(){
			...
			$form_data .= '<input placeholder="'.((isset($this->pr_option['login_password_placeholder']) && !empty($this->pr_option['login_password_placeholder']))? __($this->pr_option['login_password_placeholder'],"piereg") : "").'" type="password" value="" class="input input_fields piereg_validate[required]" id="user_pass" name="pwd">';
			$form_data .= '</div>';
			return $form_data;
		}
		function add_rememberme(){
			$form_data = '<p class="forgetmenot">';
				...
				$form_data .= '<label for="rememberme">';
					$form_data .= __("Remember Me","piereg");
				$form_data .= '</label>';
			$form_data .= '</p>';
			return $form_data;
		}
