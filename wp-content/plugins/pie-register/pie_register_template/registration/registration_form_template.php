...
class Registration_form_template extends Registration_form
{
	...
	function addPassword($fromwidget,$field_status = "")
	{
		...
		if( isset($this->field['required']) && $this->field['required'] != "" )
			...
			$data .= '<input id="confirm_password_'.$this->id.'" type="password" data-errormessage-value-missing="'.$this->field['validation_message'].'" data-errormessage-range-underflow="'.$this->field['validation_message'].'" data-errormessage-range-overflow="'.$this->field['validation_message'].'" class="input_fields '.$this->field['css'].' piereg_validate[minSize[8],required,equals['.$this->id.']]  prPass2" placeholder="'.$this->field['placeholder2'].'" autocomplete="off" />';	
			$data .= $this->addDesc();
			$data .= '</div>';
	...
	function addPricing() //stripe_changes
	{
		...
		if ($this->check_enable_payment_method() == "true" && isset($this->field['allow_payment_gateways']) && !empty($this->field['allow_payment_gateways']) )
		{
			...
			if( count( $this->field['allow_payment_gateways'] ) == 1 ) {				
				...
				$data .= "<label>".__("Select Payment","piereg")." <span class='piereg_field_required_label'>*</span></label>";
				$data .= '<select id="select_payment_method" name="select_payment_method">';
				$data .= '<option value="">'.__("Select Payment Method","piereg").'</option>';
	...
	function addSubmit($options = array())
	{
		...
		$data .= '<input name="pie_submit" class="pie_submit" type="submit" value="'.__($this->field['text'],"piereg").'" />';		// stripe_changes	
