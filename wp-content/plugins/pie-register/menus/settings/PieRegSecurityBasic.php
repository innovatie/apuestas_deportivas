...
<form method="post" action="" id="piereg_form_general_settings_page" onsubmit="return validateSettingsSecurity();">
  ...
  <div class="fields piereg_captcha_type_show" <?php echo ((!isset($piereg['captcha_in_login_value']) || isset($piereg['captcha_in_login_value']) && $piereg['captcha_in_login_value'] == 0 )?'style="display:none;"':'') ?>>
    ...
    <?php _e("<strong>Note:</strong> Classic ReCaptcha will not show multiple times on a single page.",'piereg') ?>
      ...
      <div class="fields">
        ...
        <?php _e("invalid login attempts",'piereg') ?>. <?php echo $_available_in_pro; ?></div> </div>
  ...
  <div class="fields piereg_captcha_forgot_pass_type_show" <?php echo (!isset($piereg['captcha_in_forgot_value']) || (isset($piereg['captcha_in_forgot_value']) && $piereg['captcha_in_forgot_value'] == 0 )?'style="display:none;"':'') ?>>
    ...
    <?php _e("<strong>Note:</strong> Classic ReCaptcha will not show multiple times on a single page.",'piereg') ?>
      ...
      <div class="fields">
        ...
        <?php _e("invalid login attempts",'piereg') ?> . <?php echo $_available_in_pro; ?></div>
        </div>
  ...
  <div class="fields">
    ...
    <div>
      <select name="verification" id="verification_2" >
        ...
        <?php _e("Admin Approval AND Verify Email Address","piereg"); echo ($this->piereg_pro_is_activate)?'': __(" - [PRO]","piereg"); ?>
        </option>
      </select>
    </div>
    <div class="verification_data"> <span>
      ...
      <p>
        <?php _e("User Ids that are not verified within the grace period are deleted at next logon attempt. 0 (Zero) for Unlimited",'piereg') ?>
      </p>
    </div>
  </div>
  <div class="fields">
  ...
  <hr class="seperator" />
  <h3>
    <?php _e("Restrict Website Content For Search Engine(s) / Bot",'piereg'); ?>
  </h3>
  <div class="fields">
    <div class="radio_fields">
    ...
    <label for="captcha_publc" class="label_mar_top">
      <?php _e("Restrict search engines and bots from crawling page content.",'piereg') ?>
    </label>
  </div>
  <div class="fields">
    <label for="restrict_bot_content" class="label_textarea">
      <?php _e("User Agents to Reject",'piereg') ?>
    </label>
