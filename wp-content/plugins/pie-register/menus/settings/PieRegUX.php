...
<?php if( (isset($_GET['tab']) && $_GET['tab'] == 'ux') && (isset($_GET['subtab']) && $_GET['subtab'] == 'advanced')) { ?>
        <?php 	$_disable = "";
			if(!$this->piereg_pro_is_activate)	{
				$_disable 			= "disabled";
				$_available_in_pro 	= ' - <span style="color:red;font-size:14px;">'. __("Available in Pro version","piereg") . '</span>';
			}
			?>
        <h3><?php echo __("Pie-Register Theme",'piereg') . $_available_in_pro ?></h3>
        <div class="fields">
        ...
        <div class="fields fields_submitbtn">
            ...
            <input <?php echo $_disable; ?> type="submit" class="submit_btn" value="<?php _e("Save Settings","piereg"); ?>" />
        </div>        
<?php } else { ?>	
    ...
    <?php if ( $piereg['custom_logo_url'] ) {?>
        <div class="fields">
            ...
            <img class="mar_as_allinputs" src="<?php echo $piereg['custom_logo_url'];?>" alt="<?php _e( 'Custom Logo', 'piereg' ); ?>" />
        </div>
        <div class="fields">
