...
/* 
Plugin Name: Pie Register - Custom Registration Form and User Login WordPress Plugin
Plugin URI: https://pieregister.com/
Description: User registration forms, conditional logic, invitation codes, 2-step verification, IP blacklisting, accept payments and more!
Version: 3.0.20
	...
	class PieRegister extends PieReg_Base{
		...
		function __construct(){
			...
			*/
			...
			if( is_admin() && ( (isset($_POST['piereg_license_key']) && !empty($_POST['piereg_license_key'])) || $this->piereg_pro_is_activate || (isset($_POST['piereg_pro_not_activated'] )) ) )
			{
				...
				$this->load_plugin_self_updater($plugin_name);
			}
		...
		function pie_main(){
			...
			*/
			...
			if(isset($_GET['prfrmid']) and ((int)$_GET['prfrmid']) != 0 and isset($_GET['status']) and $_GET['status'] != "")
			{
				...
				#$this->pie_post_array['notice'] = __("Successfully Change Status of Pie Register Registration Form","piereg");
			...
			if( ( isset($_GET['prfrmid']) and ((int)$_GET['prfrmid']) != 0 and isset($_GET['action']) and $_GET['action'] == "delete" ) && $this->check_if_role_admin() )
			{
				...
				if(count($pr_all_form_info) > 1) {
					...
					$this->pie_post_array['error_message'] = __("Form deletion failed.","piereg");
				}
			}
		...
		*/
		...
		function piereg_register_scripts(){
			...
			wp_register_script('prBackendCustomLogo_script',plugins_url("/js/prBackendCustomLogo.js",__FILE__),'','',true);
			...
			wp_register_style( 'pie_ui_css', plugins_url("/css/pie_ui.css",__FILE__),false,'2.0', "all" );
		...
		function piereg_backendregister_scripts(){
			...
			wp_register_style( 'pie_go_pro_menu_css', plugins_url("/css/admin-go-pro-styles.css",__FILE__),false,'1.0', "all" );
			//Scripts
		...
		function print_multi_lang_script_vars(){
			...
			$this->pie_pr_dec_vars_array = array(
				...
				'ValidationMsgText24'					=> __("* Invalid Phone No. Allowed Format (xxx) xxx-xxxx","piereg"),
		...
		function pie_frontend_enqueu_scripts(){
			...
			if(isset($piereg_global_options['outputjquery_ui']) && $piereg_global_options['outputjquery_ui'] == 1){
				...
				wp_register_script('jquery-ui-core', '//ajax.googleapis.com/ajax/libs/jqueryui/1.12.0/jquery-ui.min.js',array('jquery'),'1.8.0',false);
				wp_enqueue_script( 'jquery-ui-core' );
			}			
		...
		function add_action_links( $links, $file ) 
		{
			...
			$links[] = '<a style="color:#13ad11;" target="_blank" href="https://pieregister.com/">'.__("Go Pro","piereg").'</a>';
		...
		*/
		...
		{
			$show_release_notice   = get_option( 'pie_promo_notice_for_pro' );
			if ( empty( $show_release_notice ) ) {
				
				?>
                <div class="notice notice-info pie-admin-notice is-dismissible">
                	<p><?php echo ( _e( 'Authorize.net Addon is now available for $14.99 ','pireg')); ?></p>
                	<p><a href="http://pieregister.com/" target="_blank" id="pie_opt_in_start" class="button button-primary"><?php _e( 'Purchase Now', 'piereg' ) ?></a>&nbsp;<a href="javascript:void(0);" class="button-secondary pie_opt_in_link"><?php _e( 'No thanks', 'piereg' ) ?></a></p>
                </div>
		...
		{
			update_option( 'pie_promo_notice_for_pro', 'no' );
		}
		
		public function pie_promo_notice_for_free()
		{
			$show_release_notice   = get_option( 'pie_promo_notice_for_free' );
			if ( empty( $show_release_notice ) ) {
				?>
                <div class="notice notice-info pie-admin-notice is-dismissible">
                	...
                	<p><?php echo ( _e( 'Thanks for installing Pie Register!<br /> We hope you like the plugin. Check plugin Pro <a href="admin.php?page=pie-pro-features"><strong>Features</strong></a> and <a href="admin.php?page=pie-pro-features&tab=addons"><strong>Addons</strong></a> from here.<br /><br /><strong>Pie Register offers perpetual licensing - purchase once and use for a lifetime, no hassle or recurring periodic payments</strong>.','piereg')); ?></p>
                	<p><a href="https://pieregister.com/plan-and-pricing/" target="_blank" id="pie_opt_in_start" class="button button-primary"><?php _e( 'Upgrade Now', 'piereg' ) ?></a>&nbsp;<a href="javascript:void(0);" class="button-secondary pie_opt_in_link"><?php _e( 'No thanks', 'piereg' ) ?></a></p>
                </div>
                <script type="text/javascript">
					jQuery(document).on('click', '#pie_opt_in_start', function (e) {
						jQuery(this).parents('.pie-admin-notice').find( '.notice-dismiss' ).trigger('click');
		...
		{
			update_option( 'pie_promo_notice_for_free', 'no' );
		}
		...
		function myplugin_meta_box_callback( $post ) { 
			...
			<?php $result = get_post_meta( $post->ID, '_piereg_post_restriction', true );?>
                ...
                <?php if( $this->piereg_pro_is_activate ) { ?>
                    ...
                    <div class="piereg_restriction_field_area">
                        <div class="piereg_label">
                            <label><?php _e( 'Enable', 'piereg' ); ?></label>
                        </div>
                        <div class="piereg_input">
...
?>
		...
		function checkLogin(){
									...
									if( ($cookie_result1 == $cookie_result2) && ($cookie_result3 == $this->pie_post_array['piereg_math_captcha_login'])){
										...
										if($this->piereg_authentication($this->pie_post_array['log'],$this->pie_post_array['pwd'])){
											...
											if( $piereg_security_attempts_login_value ){
												$errors->add('login-error',apply_filters("blocked_ip_due_to_many_attempts_failed",__("We are sorry, but this IP address has been blocked due to too many recent failed login attempts.","piereg")));
											}
										}
										$error_found++;
									}
					...
					if($error_found == 0){
						...
						else
						{
							...
							if($this->piereg_authentication($this->pie_post_array['log'],$this->pie_post_array['pwd'])){
								...
								if( $piereg_security_attempts_login_value ){
									$user = new WP_Error('piereg_authentication_failed', __("We are sorry, but this IP address has been blocked due to too many recent failed login attempts.","piereg"));
								}
							}
						}
						...
						if ( is_wp_error($user))
						{
							...
							{
								$user_login_error = apply_filters('pie_invalid_username_password_msg_txt','<strong>'.ucwords(__("error","piereg")).'</strong>: '.__("Invalid username or password","piereg").'. <a href="'.$this->pie_lostpassword_url().'" title="'.__("Password Lost and Found","piereg").'">'.__("Lost your password?","piereg").'</a>');
							}else if(strpos(strip_tags($user_login_error),'password you entered',9) > 10)
							{
								$user_login_error = apply_filters('pie_invalid_user_password_msg_txt','<strong>'.ucwords(__("error","piereg")).'</strong>: '.__("Invalid username or password","piereg").' <strong>'.$this->pie_post_array['log'].'</strong> '.__("is incorrect","piereg").'. <a href="'.$this->pie_lostpassword_url().'" title="'.__("Password Lost and Found","piereg").'">'.__("Lost your password?","piereg").'</a>');
							}
								...
								if(!$this->deleteUsers($user->ID,$user->user_email,$user->user_registered)){
									if($active == "0")//If not active
									{
										...
										if((($this->check_enable_payment_method()) == "true") && ("not" === "using") )
										{
											...
											$this->pie_post_array['error'] = __('Please Renew your account',"piereg");
			...
			{
				$errors->add('login-error',apply_filters("piereg_something_went_wrong_msg",__('Something went wrong.','piereg')));	
			}
		}
		...
		function AddPanel()
		{
			...
			$pie_page_suffix_1 = add_menu_page( "Pie Register", __('Pie Register',"piereg") , 'manage_options', 'pie-register',  array($this,'PieRegNewForm'), plugins_url("/images/pr_icon.png",__FILE__), 60 );
			...
			*/
			...
			if( !$this->piereg_pro_is_activate )
			{
				$pie_page_suffix_13 = add_submenu_page( 'pie-register', 'Go Pro!', '<span style="color: #C30604">'. __('Go Pro!',"piereg") .'</span>' , 'manage_options', 'pie-pro-features', array($this, 'PieRegProFeatures'));
		...
		function saveFields()
		{
			...
			if(isset($_POST['piereg_reg_form_nonce']) && wp_verify_nonce( $_POST['piereg_reg_form_nonce'], 'piereg_wp_reg_form_nonce' ))
			{
				...
				$this->pie_post_array['success_message'] = __("Settings Saved","piereg");
			}
			else{
				$this->pie_post_array['error_message'] = __("Wrong nonce","piereg");
			}
		}
		...
		function check_register_form()
		{
			...
			else if(($this->check_enable_payment_method()) == "true")
			{
				...
				else
				{
					...
					$this->pie_post_array['error'] = __("Please select any payment method","piereg");	
				}
			}
			else if(trim($wp_session['payment_error']) != "")
			{
				$this->pie_post_array['error'] = __($wp_session['payment_error'],"piereg");
			...
			if(sizeof($errors->errors) == 0)
			{
				...
				*/
				...
				else if($option_user_verification == 1 )//Admin Verification
				{
					...
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration_admin_veri",__("Thank you for your registration. You will be notified once the admin approves your account.",'piereg'));
				}
				...
				else if($option_user_verification == 2 )//E-Mail Link Verification
				{
					...
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration_email_veri",__("Thank you for your registration. An activation link has been sent to verify your email address.",'piereg'));
				...
				else if($option_user_verification == 3 )//Admin & E-Mail Verification
				{
					...
					$this->pie_post_array['registration_success'] = apply_filters("piereg_thank_you_for_your_registration_admin_email_veri",__("Thank you for your registration. You will be notified once the admin approves your account",'piereg'));
				}
				...
				{					
					$this->pie_post_array['registration_success'] = apply_filters("piereg_payment_success",__($wp_session['payment_success'],"piereg"));
					$wp_session['payment_error'] = "";
					$wp_session['payment_success'] = "";
				}
				else if($confirmation_type == "" || $confirmation_type == "text" )
				{
					if(empty($this->pie_post_array['registration_success']))
						$this->pie_post_array['registration_success']	= __($form->data['submit']['message'],"piereg");
				}
		...
		function verifyUsers()
		{
			...
			if(isset($_POST['piereg_verifyit_nonce']) && wp_verify_nonce( $_POST['piereg_verifyit_nonce'], 'piereg_wp_verifyit_nonce' ))
			{
				...
				if($valid)
				{
					...
					$this->pie_post_array['notice'] = "<strong>".__('error','piereg').":</strong>".__("Please select a user to send emails to", "piereg");
			}
			else{
				$this->pie_post_array['error_message'] = __("Wrong nonce","piereg");
			}
		...
		function PaymentLink()
		{
			...
			if(isset($_POST['piereg_verifyit_nonce']) && wp_verify_nonce( $_POST['piereg_verifyit_nonce'], 'piereg_wp_verifyit_nonce' ))
			{
				...
				if( is_array($valid)) 
				{
					...
					if($sent > 0)
						$this->pie_post_array['notice'] = __("Payment Link Emails have been re-sent", "piereg");
					else
						$this->pie_post_array['notice'] = __("Invalid User Types", "piereg");
		...
		function AdminEmailValidate()
		{
			...
			if(isset($_POST['piereg_verifyit_nonce']) && wp_verify_nonce( $_POST['piereg_verifyit_nonce'], 'piereg_wp_verifyit_nonce' ))
			{
				...
				if($sent > 0)
						$this->pie_post_array['notice'] = __("Verification Emails have been re-sent", "piereg");
					else
						$this->pie_post_array['notice'] = __("Invalid User Types", "piereg");
				}
				else
		...
		function PieRegSettingsProcess() {
			...
			if(isset($_POST['piereg_settings_allusers']) && wp_verify_nonce( $_POST['piereg_settings_allusers'], 'piereg_wp_settings_allusers' )) {
				...
				$update['alternate_login'] 			= intval($this->pie_post_array['alternate_login']);
				$update['alternate_register'] 		= intval($this->pie_post_array['alternate_register']);
				$update['alternate_forgotpass'] 	= intval($this->pie_post_array['alternate_forgotpass']);
				$update['alternate_profilepage']	= intval($this->pie_post_array['alternate_profilepage']);
				$update['after_login']				= intval($this->pie_post_array['after_login']);
				$update['alternate_login_url']		= $this->pie_post_array['alternate_login_url'];
				$update['alternate_logout']			= intval($this->pie_post_array['alternate_logout']);
				...
				$this->pie_post_array['error'] = __("Wrong nonce","piereg");				
			}
			...
			if(!isset($this->pie_post_array['error']) && empty($this->pie_post_array['error']))
				$this->pie_post_array['notice'] = apply_filters("piereg_settings_saved",__('Settings Saved', 'piereg'));
		...
		function PieRegRedirectSettingsAction(){
			...
			if( isset($this->pie_post_array['redirect_settings_status_id']) && !empty($this->pie_post_array['redirect_settings_status_id']) ){
				if($wpdb->query($wpdb->prepare("update `".$piereg_table_name."` SET `status` = CASE WHEN status = 1 THEN  0 WHEN status = 0 THEN 1 ELSE  0 END  WHERE `id` = %d",intval($this->pie_post_array['redirect_settings_status_id']))))
					$this->pie_post_array['notice'] = __("Status has been changed.","piereg");
				else
					$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
			}
			/*	Delete Record	*/
			elseif(isset($this->pie_post_array['redirect_settings_del_id'])){
				if($wpdb->query($wpdb->prepare("DELETE FROM `".$piereg_table_name."` WHERE `id` = %d", intval($this->pie_post_array['redirect_settings_del_id']))))
					$this->pie_post_array['notice'] = __("Record has been deleted","piereg");
				else
					$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
			}
			/*	Add Record	*/
			elseif(isset($this->pie_post_array['redirect_settings_add_new']))
			{
				...
				if(isset($_POST['piereg_redirect_settings_nonce']) && wp_verify_nonce( $_POST['piereg_redirect_settings_nonce'], 'piereg_wp_redirect_settings_nonce' ))
				{
					...
					{
						$this->pie_post_array['error'] = __("Please select any page or url for user role.","piereg");	
					}
					elseif(
						...
						if($wpdb->query( $wpdb->prepare($sql, $insr_piereg_user_role, $insr_logged_in_url, $insr_log_in_page, $insr_log_out_url, $insr_log_out_page, $insr_status) ))
							$this->pie_post_array['notice'] = __("Successfully add new record","piereg");
						else
							$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
					}else{
						$this->pie_post_array['error'] = __("Invalid field(s)","piereg");
					}
				}else{
					$this->pie_post_array['error'] = __("Wrong nonce","piereg");
				}
			...
			elseif(isset($this->pie_post_array['redirect_settings_update'])){
				...
				if(isset($this->pie_post_array['id']) && $this->pie_post_array['id'] && isset($this->pie_post_array['logged_in_url']) && isset($this->pie_post_array['log_out_url'])){
					...
					if( $wpdb->query( $wpdb->prepare($sql, $upd_logged_in_url, $upd_log_in_page, $upd_log_out_url, $upd_log_out_page, $upd_id) ) )
						$this->pie_post_array['notice'] = __("Successfully update record","piereg");
					else
						$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
				}else{
					$this->pie_post_array['error'] = __("Invalid field(s)","piereg");
			...
			if( isset($_POST['piereg_invitation_code_nonce']) && wp_verify_nonce( $_POST['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ) )
			{
				...
				if( isset($this->pie_post_array['invi_del_id']) ) 
				{
					...
					if(isset($this->pie_post_array['piereg_invitation_code_nonce']) && wp_verify_nonce( $this->pie_post_array['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ))
					{
						if($wpdb->query( $wpdb->prepare("DELETE FROM ".$codetable." WHERE id = %s", $this->pie_post_array['invi_del_id']) ))	
							$this->pie_post_array['notice'] = __("The Invitation Code has been deleted","piereg");
						else
							$this->pr_error_log($wpdb->last_error.($this->get_error_log_info(__FUNCTION__,__LINE__,__FILE__)));
					}else{
						$this->pie_post_array['error'] = __("Wrong nonce","piereg");
				...
				else if( isset($this->pie_post_array['piereg_codepass']) ) 
				{
					...
					if(isset($this->pie_post_array['piereg_invitation_code_nonce']) && wp_verify_nonce( $this->pie_post_array['piereg_invitation_code_nonce'], 'piereg_wp_invitation_code_nonce' ))
					{
						...
						if(isset($this->pie_post_array['save_submit'])){
							...
							if(update_option(OPTION_PIE_REGISTER,$piereg)){
								$this->pie_post_array['notice'] = __("Settings Saved","piereg");
							}
						}
						...
						if(isset($this->pie_post_array['add_code'])){
							if(empty($this->pie_post_array['piereg_codepass']) || trim($this->pie_post_array['piereg_codepass']) == ''){
								$this->pie_post_array['error'] = __("Code field should not be empty.","piereg");
							}
							...
							if(isset($this->pie_post_array['invitation_code_usage']) && !is_numeric($this->pie_post_array['invitation_code_usage']) && trim($this->pie_post_array['invitation_code_usage']) != ''){
								...
								$this->pie_post_array['error'] .= __("Usage only allows numeric characters.","piereg");
							}elseif(isset($this->pie_post_array['invitation_code_usage']) && $this->pie_post_array['invitation_code_usage'] < 0){
								if(empty($this->pie_post_array['piereg_codepass']) || trim($this->pie_post_array['piereg_codepass']) == ''){
									$this->pie_post_array['error'] .= '<br/>';
								}
								$this->pie_post_array['error'] .= __("Usage should not be less than 0 (zero).","piereg");
							}
											
							$update["codepass"] = $this->pie_post_array['piereg_codepass'];
							...
							if(isset($special_char) && $special_char){
								$this->pie_post_array['error'] = __("Special Characters are not allowed in code field.","piereg");
							}
							...
							if(!$codeadded && $count_code != $count_added_code && !isset($this->pie_post_array['error'])){
								...
								$this->pie_post_array['notice'] = $count_not_added_code.__(" invitation Code(s) already exists","piereg");
							}elseif($codeadded) {
								if(isset($this->pie_post_array['invitation_code_usage']) && is_numeric($this->pie_post_array['invitation_code_usage']) && $this->pie_post_array['invitation_code_usage'] >= 0)
								{
									...
									if(update_option(OPTION_PIE_REGISTER,$piereg))
									{
										$this->pie_post_array['notice'] = __("Invitation Code(s) added successfully","piereg");
									}
								}
								
								if($count_code != $count_added_code){
									$count_not_added_code = $count_code - $count_added_code - $count_special_char;
									$this->pie_post_array['notice'] = $count_added_code.__(" invitation Code(s) added successfully.","piereg");
									if($count_not_added_code != 0){
										$this->pie_post_array['notice'] .= '<br/>'.$count_not_added_code.__(" invitation Code(s) already exists","piereg");
									}
								}else{						
									$this->pie_post_array['notice'] = $count_added_code.__(" invitation Code(s) added successfully","piereg");
								}
		...
		{	
			...
			if(isset($_POST['piereg_export_users_nonce']) && wp_verify_nonce( $_POST['piereg_export_users_nonce'], 'piereg_wp_exportusers_nonce' ))
			{
				...
				{
					$this->pie_post_array['error_message'] = __("No Record Found","piereg");
				}
			}else{
				$this->pie_post_array['error_message'] = __("Wrong nonce","piereg");
			}
		...
		function importUsers()
		{
			if(isset($_POST['piereg_import_users_nonce']) && wp_verify_nonce( $_POST['piereg_import_users_nonce'], 'piereg_wp_importusers_nonce' ))
			{
				...
				{
					$this->pie_post_array['error_message'] = apply_filters("piereg_didnt_select_file_to_import",__("You did not select a file to import users",'piereg'));
					return;	
				}
				$ext = pathinfo($_FILES['csvfile']['name'], PATHINFO_EXTENSION);
				if($ext != "csv")
				{
					$this->pie_post_array['error_message'] = __("Invalid CSV file.",'piereg');	
					return;	
				}
				...
				{
					 $this->pie_post_array['error_message'] = __("Invalid CSV File. It must contain all the default user fields.",'piereg');
					 return;	
				}
				$table_fields = array(
									  //////////// DEFAULT FEILDS //////////
				...
				if($success_import)
					$this->pie_post_array['success_message'] = __("$success_import user(s) imported.",'piereg');				
				
				if($unsuccess_import)
					$this->pie_post_array['error_message'] = __("$unsuccess_import user(s) do not imported.",'piereg');
					
				if($already_exist){
					if(isset($this->pie_post_array['update_existing_users']) && $this->pie_post_array['update_existing_users'] == "yes"){
						$this->pie_post_array['success_message'] .= "<br />".__("$already_exist user(s) Update.",'piereg');
					}else{
						$this->pie_post_array['error_message'] = __("$already_exist user(s) already exist.",'piereg');
					}
				}
			}else{
				$this->pie_post_array['error_message'] = __("Wrong nonce","piereg");
		...
		function SaveSettings()
		{
			...
			if( isset($_POST['action']) && $_POST['action'] == 'pie_reg_update' )
			{
				if(isset($_POST['payment_gateway_page'],$_POST['piereg_payment_gateway_page_nonce']) && wp_verify_nonce($_POST['piereg_payment_gateway_page_nonce'], 'piereg_wp_payment_gateway_page_nonce') )
				{
					...
					$payment_success_msg			= trim(((isset($this->pie_post_array['payment_success_msg']) && !empty($this->pie_post_array['payment_success_msg']))?$this->pie_post_array['payment_success_msg']:__("Payment was successful.","piereg")));
					$update["payment_success_msg"]	= $this->disable_magic_quotes_gpc($payment_success_msg);
					
					$payment_faild_msg				= trim(((isset($this->pie_post_array['payment_faild_msg']) && !empty($this->pie_post_array['payment_faild_msg']))?$this->pie_post_array['payment_faild_msg']:__("Payment failed.","piereg")));
					$update["payment_faild_msg"]	= $this->disable_magic_quotes_gpc($payment_faild_msg);
					
					$payment_renew_msg				= trim(((isset($this->pie_post_array['payment_renew_msg']) && !empty($this->pie_post_array['payment_renew_msg']))?$this->pie_post_array['payment_renew_msg']:__("Account needs to be activated.","piereg")));
					$update["payment_renew_msg"]	= $this->disable_magic_quotes_gpc($payment_renew_msg);
					
					
					$payment_already_activate_msg	= trim(((isset($this->pie_post_array['payment_already_activate_msg']) && !empty($this->pie_post_array['payment_already_activate_msg']))?$this->pie_post_array['payment_already_activate_msg']:__("Account is already active.","piereg")));
		...
		*/
		...
		function addCaptcha($field,$no)
		{
			...
			echo '<p>'.'<strong>'.__("Note","piereg").':</strong> '.__("Please make sure that Re-captcha keys are entered in Settings page","piereg").'</p>';
			echo '<input type="hidden" id="default_'.$field['type'].'">';	
		}
		...
		function deleteUsers($user_id = 0,$user_email = "",$user_registered = "")
		{
			...
			if( ($grace != 0 and $user_id != 0) and ($user_email != "" and $user_registered != "") and ($register_type != '' and $register_type != "admin_verify") && $is_active	== 0 )
			{
				...
				if($user_registered < $date)
				{
					...
					$errors->add('login-error',apply_filters("piereg_msg_deleted_unverified_user",__('Your account has been removed due to not verifying it within the grace period.','piereg')));
		...
		function showForm($id="",$title="true",$description="true")
		{
			...
			if(isset($this->pie_post_array['success']) && $this->pie_post_array['success'] != "")
				$output .= '<p class="piereg_message">'.apply_filters('piereg_messages',__($this->pie_post_array['success'],"piereg")).'</p>';
			if(isset($this->pie_post_array['error']) && $this->pie_post_array['error'] != "")
				$output .= '<p class="piereg_login_error">'.apply_filters('piereg_messages',__($this->pie_post_array['error'],"piereg")).'</p>';
			
			if(isset($this->pie_post_array['registration_success']) && $this->pie_post_array['registration_success'] != ""){
				$output .= '<p class="piereg_message">'.apply_filters('piereg_messages',__($this->pie_post_array['registration_success'],"piereg")).'</p>';
				unset($_POST);
			}
		...
		function showLoginForm()
		{
			...
			{
				$this->pie_post_array['warning'] = (__("Please renew your account","piereg"));
		...
		function showForgotPasswordForm()
		{
			...
			$option 		= get_option(OPTION_PIE_REGISTER);
			if(!is_admin() && is_user_logged_in() && $option['redirect_user']==1 )
			{
				$this->afterLoginPage();
				return "";	
			}	
		...
		function Add_payment_option($send_data,$paymeny_gatways) // Only For Paypal
		{
			...
			if($check_payment["enable_paypal"] == 1 && !(empty($check_payment['paypal_butt_id'])) )
			{
				...
				{
					$send_data .= '<option data-img="'.plugins_url('pie-register/images/paypal_std_btn.png').'" value="PaypalStandard">'.(__("Paypal (Standard)","piereg")).'</option>';
				}
			}
				...
				if( isset($_POST['select_payment_method']) and trim($_POST['select_payment_method']) != "" )
				{
					...
					if ( is_wp_error($user))
					{
						...
						if(strpos(strip_tags($user_login_error),'Invalid username',5) > 6){
							$user_login_error = '<b>'.ucwords(__("error","piereg")).'</b>: '.__("Invalid username or password","piereg");
						}
						else if(strpos(strip_tags($user_login_error),'password you entered',9) > 10){
							$user_login_error = '<strong>'.ucwords(__("error","piereg")).'</strong>: '.__("Invalid username or password","piereg");
						}
					...
					elseif($user->ID != 0 or $user->ID != ""){
						...
						if($user_meta['active'][0] == 0){
							...
							$this->pie_post_array['success'] = apply_filters("you_are_already_active",__($piereg['payment_already_activate_msg'],"piereg"));
						}
					}
					else{
						$this->pie_post_array['error'] = apply_filters("piereg_Invalid_Username_or_Password",__("Invalid Username or Password","piereg"));
					}
				}
				else{
					$this->pie_post_array['error'] = apply_filters("piereg_Please_Select_any_payment_method",__("Please Select any payment method","piereg"));
				}
			} else {
		...
		function wp_mail_send($to_email = "",$key = "",$additional_msg = "",$msg = "",$email_variable = array())
		{
			...
			if(trim($to_email) != "" and trim($key) != "" )
			{
				...
				if(!wp_mail($to,$subject,$message,$headers))
				{
					...
					if(trim($msg) != "")
					{
						$this->pie_post_array['success'] = __($msg,"piereg");
					}
				}
			}
		}
			...
			if(isset($_POST['import_export_settings']) && $_POST['import_export_settings']){
				...
				elseif( isset($_POST['import_general_settings']) && isset($_FILES['import_general_settings_file']) && $_FILES['import_general_settings_file']['name'] != "" ){
					...
					if(pathinfo($_FILES['import_general_settings_file']['name'],PATHINFO_EXTENSION) == "json"){
						...
						$this->pie_post_array['error'] = __("Invalid File","piereg");
					}
				}
			...
			if(isset($_POST['piereg_import_general_settings']) && wp_verify_nonce( $_POST['piereg_import_general_settings'], 'piereg_wp_import_general_settings' ))
			{
				...
				if(isset($array_json_file['piereg_plugin_db_version']) && !empty($array_json_file['piereg_plugin_db_version'])){
					...
					if($pie_plugin_db_version != $import_file_db_version){
						$this->pie_post_array['error'] = __("The file version is not compatible with this version","piereg");
						return false;
					}
				}else{
					$this->pie_post_array['error'] = __("The file version is not compatible with this version","piereg");
						...
						case "pie_us_states":
							if($arr_val){
								...
								if(is_array($result_merge_array)){
									...
									$this->pie_post_array['success_message'] = __("Successfully import all settings","piereg");
								}
								unset($db_option);
		...
		function export_invitation_codes($is_download = true){
			if(isset($_POST['piereg_export_invitations_codes']) && wp_verify_nonce( $_POST['piereg_export_invitations_codes'], 'piereg_wp_export_invitations_codes' ))
			{
				...
				if(!empty($result_invitaion_code)){
					...
					$this->pie_post_array['error'] = __("Record(s) not found","piereg");
				}
			}else{
				$this->pie_post_array['error_message'] = __("Wrong nonce","piereg");
			}
		...
		function import_invitation_codes(){
			if(isset($_POST['piereg_import_invitations_codes']) && wp_verify_nonce( $_POST['piereg_import_invitations_codes'], 'piereg_wp_import_invitations_codes' ))
			{
				...
				if(isset($array_json_file['invitation_codes']) && is_array($array_json_file['invitation_codes'])){
					...
					$this->pie_post_array['success_message'] = __("Successfully import invitation codes","piereg");					
				...
				if(isset($array_json_file['admin_email_template'],$array_json_file['user_email_template'])){
					if(is_array($array_json_file['admin_email_template']) && is_array($array_json_file['user_email_template'])){
						...
						if(is_array($merge_email_templates)){
							...
							$this->pie_post_array['success_message'] = __("Successfully import e-mail templates","piereg");
						}
					}
				}
			}else{
		...
		*/
		...
		function piereg_plugin_row_meta( $links, $file ) {
			if ( $file == PIEREG_PLUGIN_BASENAME ) {
				$row_meta = array(
					'documentation'		=>	'<a href="' . esc_url( apply_filters( 'pieregister_docs_url', 'https://pieregister.com/manual/introduction/' ) ) . '" title="' . esc_attr( __( 'View Pie Register Documentation', 'piereg' ) ) . '" target="_blank">' . __( 'Documentation', 'piereg' ) . '</a>',
			...
			if( isset($_POST['piereg_activate_addon_license_key'], $_POST['save_addon_license_key_settings']) )
			{
				if(isset($_POST['piereg_addon_license_key_nonce']))
				{
					if(wp_verify_nonce( $_POST['piereg_addon_license_key_nonce'], 'piereg_wp_addon_license_key_nonce' )){
						...
						{
							$LK_options['api_key']			= $_POST['piereg_addon_license_key'];
							$LK_options['activation_email']	= $_POST['piereg_addon_license_email'];	
						}
						else {
		...
		function piereg_payment_log_file_action(){
			...
			if( isset($_POST['piereg_download_payment_log_file']) && isset($_POST['piereg_payment_log']) && wp_verify_nonce( $_POST['piereg_payment_log'], 'piereg_wp_payment_log' ) ){
					...
					if( !empty($read_log_file) ){
						...
						$this->pie_post_array['error'] = __("Empty Payment Log File","piereg");
					}
					...
					$this->pie_post_array['notice'] = __("Successfully remove payment log","piereg");
