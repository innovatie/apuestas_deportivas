...
class Pie_Invitation_Table extends WP_List_Table
{
    ...
    {
         _e( 'No invitation codes are found', "piereg" );
    }
    ...
    public function prepare_items( )
    {
        ...
        foreach ( $posts_array as $key => $post )
        {
			...
			$posts[ $key ]->action = '<a onclick="changeStatusCode(\''.$post->id.'\',\''.$post_name.'\',\''.$title.'\');" href="javascript:;" title="'.__($title,"piereg").'" class="'.$class.'"></a> <a class="delete" href="javascript:;" onclick="confirmDelInviteCode(\''.$post->id.'\',\''.$post_name.'\');" title="'.__("Delete","piereg").'"></a>';
			$id++;
        }
