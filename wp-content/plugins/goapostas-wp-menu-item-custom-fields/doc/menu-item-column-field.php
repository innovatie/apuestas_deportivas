...
class Menu_Item_Custom_Fields_Example {
    ...
    public static function init() {
        ...
        self::$fields = array(
            'column' => __( 'Move to new column', 'menu-item-custom-fields-example' ),
            'highlighted' => __( 'Highlight this item', 'menu-item-custom-fields-example' )
        );
    }
    /**
        ...
        foreach ( self::$fields as $_key => $label ) :
            ...
            ?>
                <p class="description description-wide <?php echo esc_attr( $class ) ?>">
                    <?php printf(
                        '<label for="%1$s">%2$s<br /><input type="checkbox" id="%1$s" class="widefat %1$s" name="%3$s" value="yes" %4$s /></label>',
                        esc_attr( $id ),
                        esc_html( $label ),
                        esc_attr( $name ),
                        esc_attr( $checked )
