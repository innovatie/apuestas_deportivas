...
?>
	    <tr id='banner-<?php echo $banner['id']; ?> <?php echo $banner['type']; ?>' class='<?php echo $class.$errorclass; ?>'>
			...
			<td>
				<strong><?php if($adrotate_config['enable_editing'] == 'Y') { ?><a class="row-title" href="<?php echo admin_url('/admin.php?page=adrotate-advertiser&view=edit&ad='.$banner['id']);?>" title="<?php _e('Edit', 'adrotate-pro'); ?>"><?php echo stripslashes(html_entity_decode($banner['title']));?></a><?php } else { echo stripslashes(html_entity_decode($banner['title'])); } ?></strong>
