	...
	if ($advertisers) {
		...
		foreach($advertisers as $user_id => $user) {			
			if($adrotate_debug['publisher'] == true) {
				echo "<tr><td>&nbsp;</td><td><strong>[DEBUG]</strong></td><td colspan='3'><pre>";
				echo "User Specs: <pre>";
				echo "ID: ".$user_id;
				print_r($user); 
				echo "</pre></td></tr>"; 
		    ...
		    <tr id='adrotateindex' class='<?php echo $class; ?>'>
				<td><strong><a class="row-title" href="<?php echo admin_url('/admin.php?page=adrotate-advertisers&view=profile&user='.$user_id);?>" title="<?php _e('Edit', 'adrotate-pro'); ?>"><?php echo $user['name'];?></a></strong></td>
