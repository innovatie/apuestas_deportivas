...
function adrotate_check_schedules() {
	...
	if(!wp_next_scheduled('adrotate_empty_bin')) { // Empty the bin once a day
		wp_schedule_event($firstrun + 3600, 'daily', 'adrotate_empty_bin');
	}
...
function adrotate_core_upgrade() {
	...
	if($adrotate_version['current'] < 354) {
		$crawlers = array("Teoma", "alexa", "froogle", "bot", "inktomi","looksmart", "URL_Spider_SQL", "Firefly", "NationalDirectory","Ask Jeeves", "TECNOSEEK", "InfoSeek","www.galaxy.com", "Scooter", "Slurp", "appie", "FAST", "WebBug", "Spade", "ZyBorg", "rabaz", "Baidu", "Feedfetcher-Google", "TechnoratiSnoop", "Mediapartners-Google", "Sogou web spider", "WebAlta Crawler", "crawler", "yahoo", "msn", "ask", "ia_archiver");
		update_option('adrotate_crawlers', $crawlers);
	}
	...
	if($adrotate_version['current'] < 365) {
		add_option('adrotate_activate', array('status' => 0, 'instance' => '', 'activated' => '', 'deactivated' => '', 'type' => '', 'key' => '', 'email' => '', 'version' => ''));
	}
	...
	// 3.21
	if($adrotate_version['current'] < 381) {
		if(!wp_next_scheduled('adrotate_empty_bin')) wp_schedule_event($firstrun + 5400, 'twicedaily', 'adrotate_empty_bin');
	}
...
function adrotate_cleanup_database() {
	...
	// Delete empty ads, groups, schedules and transactions
	...
	// (Optionally) Delete binned adverts and data
	if(isset($_POST['adrotate_db_cleanup_bin'])) {
		$adverts = $wpdb->get_results("SELECT `id` FROM `{$wpdb->prefix}adrotate` WHERE `type` = 'bin';");
...
/*-------------------------------------------------------------
 Name:      adrotate_empty_bin
 Purpose:   Delete expired and binned adverts
 Since:		3.21
-------------------------------------------------------------*/
function adrotate_empty_bin() {
	global $wpdb;

	$threedaysago = adrotate_now() - 259200;

	$adverts = $wpdb->get_results("SELECT `id` FROM `{$wpdb->prefix}adrotate` WHERE `type` = 'bin' AND `updated` < {$threedaysago};");
	foreach($adverts as $advert) {
		...
		$wpdb->query("DELETE FROM `{$wpdb->prefix}adrotate_transactions` WHERE `ad` = {$advert->id};");
	}
	unset($adverts);
}
...
/*-------------------------------------------------------------
 ...
 Purpose:   Auto bin selected adverts and schedules
 Since:		4.5
-------------------------------------------------------------*/
function adrotate_auto_delete() {
	global $wpdb;

	// Auto bin expired adverts
	$now = adrotate_now();
	...
	foreach($adverts as $advert) {
		...
		if($stoptime <= $twentythreehoursago) {
			$wpdb->update("{$wpdb->prefix}adrotate", array('type' => 'bin', 'updated' => $now), array('id' => $advert->id));
		}
