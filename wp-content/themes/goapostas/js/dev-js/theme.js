...
// -----------------------------------------------
// export utility items needed by theme
// -----------------------------------------------
const goapostasJS = window.goapostasJS || {};

goapostasJS.goapostasDebounce = require('./debounce.js');
goapostasJS.goapostasUtility = require('./utility.js');

window.goapostasJS = goapostasJS;

// -----------------------------------------------
// import items needed here only
// -----------------------------------------------

const goapostasPrimaryMenu = require('./menu.js'); 
//const goapostasSlick = require('./slick.min.js');
...
jQuery(() => {
    // Set up navigation
    goapostasPrimaryMenu.setup();

    window.onscroll = function (e) {
        let top = jQuery(window).scrollTop();
        if (top > 0) {
            jQuery('.goapostas-sticky-bar').addClass('scrolled');
        } else {
            jQuery('.goapostas-sticky-bar').removeClass('scrolled');
        }
    }

    // // Set up 'goapostas utility'
    goapostasJS.goapostasUtility.isDesktop = jQuery(window).width() > 1023;

    // on resize
    jQuery(window).resize(() => {

        goapostasJS.goapostasUtility.checkDesktop();
...
jQuery(document).ready(function() {
    ...
    jQuery('.header-navigation .main-navigation > ul .header-search i.fa.fa-search').on('click', function(e){
        e.preventDefault();
        jQuery('.header-navigation .main-navigation>ul .header-search .search-container').addClass('active');
    });
    jQuery(document).keyup(function(e) {
        jQuery('.header-navigation .main-navigation>ul .header-search .search-container').removeClass('active');
    });
    jQuery('.link-to a').on('click', function(e){
        e.preventDefault();
        var link_to = jQuery(this).attr('href');
        //console.log(link_to);
        jQuery('html, body').animate({
            scrollTop: jQuery(link_to).offset().top - 120, 
        }, 500, 'linear');
    });

    if( jQuery('.sc-columns-image .col-img img').length > 0 ) {
        var cloneImage = jQuery('.sc-columns-image .col-img img').clone();
        cloneImage.addClass('float-img');
        jQuery('.sc-columns-image').prepend(cloneImage);
    }

    /**/
    jQuery('.attention-area>.wpb_column>.vc_column-inner>.wpb_wrapper>.vc_row .wpb_column > .vc_column-inner').addClass('hovered');
    jQuery('.attention-area>.wpb_column>.vc_column-inner>.wpb_wrapper>.vc_row .wpb_column > .vc_column-inner').on('hover', function(){
    	jQuery(this).toggleClass('hovered');
    });
    jQuery('.attention-area>.wpb_column>.vc_column-inner>.wpb_wrapper>.vc_row .wpb_column > .vc_column-inner').on('click', function(e){
    	e.preventDefault();
    	var link_to_go = jQuery(this).find('h6 a').attr('href');
    	//console.log(link_to_go);
    	window.location.href = link_to_go;
    });

    /* Quick nav */
    if( jQuery('section.quick-nav').length > 0 ) {
        jQuery('section.quick-nav').on('click', 'ul a', function(e) {
            e.preventDefault();

            var href = jQuery(this).attr('href'),
                target = jQuery('#unexistent_id');
    ...
    /* top casa de apostas slider */
    jQuery('.tc-apostas .vc_col-sm-4_5 .vc_column-inner > .wpb_wrapper > .vc_row-fluid').slick({
        dots: false,
        infinite: false,
        speed: 300,
        slidesToShow: 4,
        slidesToScroll: 4,
        prevArrow: jQuery('#prev-tcapostas'),
        nextArrow: jQuery('#next-tcapostas'),
        responsive: [
            {
                breakpoint: 1024,
                settings: {
    ...
    /** Top betting block */
    if( jQuery('.betting-houses-block').length > 0 ) {

        var columns = 4;
        if ( jQuery('.betting-houses-block .row-grid >.col').length == 5 ) {
            columns = 5;
        }
        jQuery('.betting-houses-block .row-grid').slick({
            dots: false,
            infinite: false,
            slidesToShow: columns,
            slidesToScroll: columns,
            prevArrow: jQuery('.betting-houses-block .s-prev'), 
            nextArrow: jQuery('.betting-houses-block .s-next'),
            responsive: [
                {
                    breakpoint: 1024,
                    settings: {
    ...
    if( jQuery('.sc-sharing').length > 0 ) {
        setTimeout(function() {
            var shareText = jQuery('.sharethis-inline-share-buttons').attr('data-share'),
                shareMore = jQuery('.sharethis-inline-share-buttons').attr('data-more'); 
            if (shareText) {

                jQuery('.sharethis-inline-share-buttons .st-btn').each(function(idx, btn) {
                    if( jQuery(btn).attr('data-network') == 'sharethis' ) {
                        jQuery(btn).find('.st-label').append(shareMore);
                    }
                    else {
                        jQuery(btn).find('.st-label').append(' '+shareText);
                    }
    ...
    if( jQuery('.hero-deg .scroll-to-sc').length > 0 ) {
        jQuery('.hero-deg .scroll-to-sc').click(function(e) {
            e.preventDefault();

            var target = jQuery('.main-content .entry-content >:first-child');
            if(target.length > 0) {
                jQuery('html, body').animate({
                    scrollTop: target.offset().top - 70, 
                }, 200, 'linear');
    ...
    // comments form validation and submit
    ...
    if( jQuery('#commentform').length > 0 ) { 
        ...
        jQuery('#commentform').on('submit', function(e) { 
            ...
            var commentForm = jQuery('#commentform'),
                text = jQuery('#commentform #comment').val().trim(),
                errorMessage = '',
                statusdiv = commentForm.find('.comment-status'),
                formdata = commentForm.serialize(),
                submitBtn = commentForm.find('#submit'); 

            submitBtn.attr('disabled', 'disabled');

            if( statusdiv.length == 0 ) { 
                commentForm.find('.comment-form-comment').after('<div class="comment-status"></div>');
                statusdiv = commentForm.find('.comment-status');
            }

            // if( !text ) {
            //     statusdiv.html('<p class="error">'+ base.comments.comment_empty +'</p>');
            //     return;
            // } 
            ...
            jQuery.ajax({
                type: 'post',
                url: base.ajax_url,
                data: formdata + '&action=ajaxcomment',
                dataType: 'json',
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    statusdiv.html('<p class="ajax-error">An error eccurs while requesting to server.</p>');
                },
                success: function(data, textStatus){
                    if(data.success == true){
                        ...
                        statusdiv.html('<p class="error">'+data.error+'</p>');
                    }
                },
                complete: function() {
    ...
    if( jQuery('.comment-navigation').length > 0 ) { 
        jQuery('.comment-navigation').on('click', '#more_comments_link', function(e) {
            e.preventDefault();
            var link = jQuery(this),
                page = link.attr( 'data-page' ),
                pid = link.attr('data-post');

            page = parseInt(page);

            jQuery('#more_comments_link').attr('disabled', 'disabled');

            jQuery.ajax({
                url : base.ajax_url,
                data : {
                    'action': 'comments_more', 
                    'pid': pid,
                    'cpage' : page,
                },
                type : 'POST',
                success : function( data ){
                    var page = parseInt( jQuery('#more_comments_link').attr('data-page') );
                    page--;
                    jQuery('#more_comments_link').attr('data-page', page);
                    jQuery('#more_comments_link').removeAttr('disabled');

                    if( page <= 0 ) {
                        jQuery('#more_comments_link').remove();
                    }

                    if( data ) {
                        jQuery('ol.comment-list').append( data );
                    } else {
                        jQuery('.comment-navigation').remove();
                    }
                }
            });
